...
os.makedirs('./data/train_image_256_128/', exist_ok=True)
os.makedirs('./data/train_label_256_128/', exist_ok=True)
# os.makedirs('./data/test_input_256_256/', exist_ok=True)

def cut_img(image_path, save_path, stride=128):
...
if __name__ == '__main__':
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=6) as executor: # windows local 환경시 max_workers=os.cpu_count()//2
        list(tqdm(
            executor.map(partial(cut_img, save_path='./data/train_image_256_128/'), train_all_input_files.to_list()),
            desc='train image cut',
            total=len(train_all_input_files)
        ))
        list(tqdm(
            executor.map(partial(cut_img, save_path='./data/train_label_256_128/'), train_all_label_files.to_list()),
